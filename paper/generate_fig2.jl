...
# Figure 1c
X = ParameterRange(:chi, L"\chi", LinRange(0.01, 0.98, 64))
Y = ParameterRange(:density, L"\overline{\rho}", LinRange(0.25, 2.0, 32))
...
ls = LinRange(0.5, 3.0, 100)
ds = LinRange(0, 1.0, 100)
chis = @. (ls^2 - ds'^2) / (ls^2 + ds'^2)

pts = lds

nx, ny = length(X.range), length(Y.range)
faces = stack(vcat(
    [[i + nx*(j-1), i+1 + nx*(j-1), i+nx + nx*(j-1)] for i in 1:nx-1 for j in 1:ny-1],
    [[i + 1 + nx*(j-1), i+ nx + 1 + nx*(j-1), i+nx + nx*(j-1)] for i in 1:nx-1 for j in 1:ny-1],
))'
...
begin 
    with_theme(manuscript_theme) do
        ...
        for i in round.(Int, LinRange(1, length(X.range), 6))
            chi = X.range[i]
        ...
        # Colorbar(fig[1,2], ax, label = L"\gamma_f")
        save(joinpath(plotdir, "havvas_fish_2.png"), fig)
        save(joinpath(plotdir, "havvas_fish_2.eps"), fig)
        fig
    end
end